$(function(){
	...
	$(".btn-danger").click(function(){
		$(".check-flight",$(this).parent().parent()).toggleClass("hidden");
	...
	$("body").delegate("button.check-manual-flight","click",function(){
		var bookingId = $(this).attr("booking-id");
		var user_flight_number = $.trim( $("input[name='flight_number']",$(this).parent().parent()).val());
		if (user_flight_number.length >3){
	...
	$("body").delegate("button.save-flight-number","click",function(){
		var bookingId = $(this).attr("booking-id");
		$.post("/save-flight-number/"+bookingId+"/"+$(this).attr("user-flight-number"),
			{csrfmiddlewaretoken:$("input[name='csrfmiddlewaretoken']").val()},function(data){
				if(data=='ok'){
